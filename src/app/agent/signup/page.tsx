...
const inputCls =
  "w-full rounded-lg bg-white/10 border border-white/20 px-3 py-2 text-sm text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-primary";
          ...
          {STEPS.map((label, i) => (
            <div key={i} className="flex-1 flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors ${
                i < step ? "bg-primary border-primary text-white" : i === step ? "border-primary text-orange-200" : "border-white/20 text-white/30"
              }`}>
                {i < step ? "✓" : i + 1}
              </div>
              <span className={`mt-1 text-[10px] text-center hidden sm:block ${i === step ? "text-orange-200" : "text-white/40"}`}>{label}</span>
              {i < STEPS.length - 1 && (
          ...
          <form onSubmit={handleNext} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select name="countryCode" value={personal.countryCode} onChange={handlePersonal}
                className="w-full rounded-lg bg-white/10 border border-white/20 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary">
                {COUNTRY_CODES.map((c) => (
