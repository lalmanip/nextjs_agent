...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as { BookingId?: unknown; Source?: unknown };
    const BookingId = String(body?.BookingId ?? "").trim();
    const Source = String(body?.Source ?? "4").trim();

    if (!BookingId) {
      return NextResponse.json({ error: "BookingId is required" }, { status: 400 });
    }

    const url = `${API_BASE_URL_MT}/flight/service/tbo/release-pnr`;
    ...
    const doFetch = (token: string) =>
      fetch(url, {
        ...
        body: JSON.stringify({ BookingId, Source }),
      });
